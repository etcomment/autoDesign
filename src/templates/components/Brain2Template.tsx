...
// Exact SVG Paths extracted from dessin-2.svg (Inkscape)
...
export function Brain2Template({ data }: { data: BrainData }): ReactElement {
  ...
  return (
    <g ref={svgRef}>
      {/* 1. SVG Arch Slices from dessin-2.svg (Interactive) */}
      <g transform={`translate(${baseTx}, ${baseTy}) scale(${baseScale}) translate(-12.41, -33.54)`}>
        {branches.map((branch, i) => {
          ...
          const sliceIdx = Math.min(5, Math.floor((i / count) * 6))
          const pathD = SLICE_PATHS[sliceIdx]!
      ...
      {/* 2. Text, Numbers (1..6) and Icons centered over Slices */}
      {branches.map((branch, i) => {
        const id = `arc-${i}`
        const isSelected = selectedIds.has(id)
        const sliceIdx = Math.min(5, Math.floor((i / count) * 6))
        const cfg = SLICE_CONFIGS[sliceIdx]!

        // Transform Inkscape coords to Canvas Space
        const ptX = baseTx + (cfg.cx - 12.41) * baseScale
        const ptY = baseTy + (cfg.cy - 33.54) * baseScale

        const aBbox = { x: ptX - 50, y: ptY - 50, width: 100, height: 100 }

        const iconKey = branch.icon ?? DEFAULT_ICONS[i % DEFAULT_ICONS.length]
        const IconFn = getDynamicIcon(iconKey)

        const textX = ptX + cfg.textDx * baseScale * 0.4
        const textY = ptY + cfg.textDy * baseScale * 0.4

        const iconX = ptX + cfg.iconDx * baseScale * 0.45
        const iconY = ptY + cfg.iconDy * baseScale * 0.45

        const numX = ptX + cfg.numDx * baseScale * 0.45
        const numY = ptY + cfg.numDy * baseScale * 0.45
        ...
        return (
          <g key={`content-${id}`} onMouseDown={e => startDrag(e, id, aBbox)} style={{ cursor: "pointer" }}>
            ...
            {/* Title & Subtitle centered in middle */}
            <text
              x={textX}
              y={textY - 6}
              textAnchor="middle"
      ...
      {/* 3. Central Head Silhouette from dessin-2.svg (Interactive) */}
      <g
