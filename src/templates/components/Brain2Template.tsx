...
function getDynamicIcon(iconName?: string, size = 26) {
...
export function Brain2Template({ data }: { data: BrainData }): ReactElement {
  ...
  // Proportional scale factor for text and icon size when N changes
  const scaleFactor = Math.min(1.2, Math.max(0.65, 6 / count))
  const iconSize = Math.round(26 * scaleFactor)
  ...
  return (
    <g ref={svgRef}>
      {/* 1. Arch Slices (Dynamic N subdivision & Proportional sizing) */}
      {branches.map((branch, i) => {
        ...
        // Position coordinates inside sector slice
        // Top-left shifted icon positioning
        const ptIcon = polarToCartesian(sCX, sCY, r + (R - r) * 0.80, midA - Math.min(6, step * 0.18))
        ...
        return (
          <g key={id} onMouseDown={e => startDrag(e, id, aBbox)} transform={getTransform(id, aBbox)} style={{ cursor: "pointer" }}>
            ...
            <text
              x={ptText.x}
              y={ptText.y - Math.round(5 * scaleFactor)}
              textAnchor="middle"
            ...
            <text
              x={ptText.x}
              y={ptText.y + Math.round(11 * scaleFactor)}
              textAnchor="middle"
              fontFamily="Arial, sans-serif"
            ...
            <text
              x={ptNum.x}
              y={ptNum.y + Math.round(7 * scaleFactor)}
              textAnchor="middle"
              fontFamily="Arial, sans-serif"
