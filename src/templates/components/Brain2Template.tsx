...
// Exact SVG Paths extracted directly from dessin-2.svg (Inkscape)
...
export function Brain2Template({ data }: { data: BrainData }): ReactElement {
  ...
  const subtitleFontSize = Math.round(11 * scaleFactor)
  ...
  return (
    <g ref={svgRef}>
      ...
      {branches.map((branch, i) => {
        ...
        const isSelected = selectedIds.has(id)
        const sliceIdx = Math.min(5, Math.floor((i / count) * 6))
        const cfg = SLICE_CONTENT_POS[sliceIdx]!
        ...
        return (
          <g key={`content-${id}`} onMouseDown={e => startDrag(e, id, aBbox)} style={{ cursor: "pointer" }}>
            ...
            <text
              x={ptTextX}
              y={ptTextY - Math.round(5 * scaleFactor)}
              textAnchor="middle"
            ...
            <text
              x={ptTextX}
              y={ptTextY + Math.round(11 * scaleFactor)}
              textAnchor="middle"
              fontFamily="Arial, sans-serif"
