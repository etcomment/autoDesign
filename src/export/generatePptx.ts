...
export { generateCanvasPptx, downloadCanvasPptx } from './svgToPptx'
...
function addShapeToSlide(slide: PptxGenJS.Slide, shape: Shape): void {
  ...
  switch (shape.type) {
    case 'rectangle': slide.addShape('rect', shapeOpts); break
    case 'ellipse': slide.addShape('ellipse', shapeOpts); break
    case 'diamond': slide.addShape('diamond', shapeOpts); break
  }
...
export async function generatePptx(model: DiagramModel): Promise<Blob> {
  ...
  for (const shape of model.shapes) addShapeToSlide(slide, shape)
  for (const conn of model.connections) addConnectionToSlide(slide, model.shapes, conn.sourceId, conn.targetId)
